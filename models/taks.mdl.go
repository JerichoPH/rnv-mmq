...
type (
	...
	TaskModel struct {
		...
		StatusCode  string          `gorm:"type:enum('ORIGINAL','PROCESSING','FINISHED','FAILED');not null;default:'ORIGINAL';comment:任务状态码" json:"status_code,omitempty"`
		StatusText  string          `gorm:"->;type:varchar(128) as ((case status_code when 'ORIGINAL' then '未执行' when 'PROCESSING' then '执行中' when 'FINISHED' then '已完成' when 'FAILED' then '失败' else '' end));comment:任务状态文本" json:"status_text,omitempty"`
